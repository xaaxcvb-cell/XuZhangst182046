...
def train_transform(resize_size=256, crop_size=224):#训练时的数据预处理
...
def test_transform(resize_size=256, crop_size=224): #测试阶段的数据预处理
    normalize = T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    return T.Compose([
        T.Resize((resize_size, resize_size)),
...
class SFUniDADataModuleBase(L.LightningDataModule): #通用数据模块基类（核心框架)
    ...
    def setup(self, stage): #加载数据集并按类别过滤
...
class DomainNetDataModule(SFUniDADataModuleBase): ###DomainNet 数据集的具体实现
    def __init__(self, batch_size, category_shift='', train_domain='sketch', test_domain='real'): #n6
        ###具体从谁到谁  clipart，painting，real，sketch
        data_dir = '/data/public/DomainNet-126/'  #n7      #'data/domainnet/'             #数据地址
...
class VisDADataModule(SFUniDADataModuleBase): #VisDA数据集的具体实现
    def __init__(self, batch_size, category_shift='', train_domain='train', test_domain='validation'):
        data_dir = '/data/public/visda-2017/'                 #数据地址   本地data/visda/
